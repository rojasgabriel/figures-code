...
function pupil_linear_shift(correctRate, norm_averagedPupil, session_borders) %TIV variable could also be pupil data

%% 1st, Linear shift test (+-200 trials)
corre_shift = nan(1, 401);
for i = 1 : 401
    a = corrcoef(norm_averagedPupil(201 : end-200), correctRate(i :  end-(401-i)));
    corre_shift(i) = a(1,2);   
end

fig = figure('Name', 'Pupil diameter linear shift analyses');
...
for i = 1 : length(session_borders)
    ...
    b = norm_averagedPupil(ttt(i): ttt(i+1)-1);
